...
Super Productivity supports six main short syntax forms for quickly adding metadata to tasks: tags (`#`), projects (`+`), project sections (`/`), time estimates (`m`/`h`), due dates (`@`), and deadlines (`!`). The project, tag, and due/deadline/time forms can be enabled or disabled in [[3.02-Settings-and-Preferences#global-settings.Short-Syntax]].
...
- **Parameters:** Section title or partial title. Matching is prefix-based, case-insensitive, with spaces removed for comparison, and falls back to the first word only (like project matching). Only sections belonging to the resolved project are considered.
- **Valid:** `"Task +Work/Design"`, `"Task +Work/des"` (prefix), `"Task /Design"` (section of the current project), `"Task /des fix login"` (first-word fallback; trailing text stays in the title).
- **Invalid / inert:** `"either/or"`, `"w/ milk"` (no word boundary before `/` or no match) — nothing is stripped from the title unless a section actually matches. `"Task / Design"` (space after `/`) is not parsed. Time forms like `1h/2h` are parsed as time before section parsing runs.
- **Enabled by:** the project Short Syntax setting (there is no separate section toggle).
- **On title edits:** editing an existing task's title with either form moves the task into the matched section (applied atomically together with any project move).
...
4. **Return value:** The `shortSyntax()` function returns an object with task changes (and `newTagTitles`, `remindAt`, `projectId`, `sectionId`) or `undefined` if nothing was parsed or syntax is disabled.
